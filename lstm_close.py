...
train = data.iloc[:int(len(data) * 0.99)]
...
#Best Hyperparameters: {'lookback': 10, 'dropout_rate': 0.1, 'epochs': 100, 'batch_size': 64, 'l2': 0.005, 'unit': 15, 'layers': 1, 'fc': 100}



##################################################
# Find the best model based on the test loss
best_model_params = {'lookback': 20, 'dropout_rate': 0.1, 'epochs': 100, 'batch_size': 8, 'l2': 0.001}

# Create the best model
best_model = LSTMModel(input_size=len(inp_columns), hidden_size=10, fcc=10, output_size=1, num_layers=1,
...
#
# for i, j in zip(y_pred_rescaled_best[-30:], y_test_rescaled[-30:]):
#     loss += abs(i-j)
# print('30 days : ',round(loss,3)/30)
